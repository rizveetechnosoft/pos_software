...
class SubscriptionController extends BaseController
{
    ...
    public function index()
    {
        ...
        $packages = Package::active()->orderby('sort_order')->take(6)->get();
    ...
    public function getSubscribeLog()
    {
        $subscribedata = Subscription::get();
        return [
            "status" => 1,
            "data" => $subscribedata
        ];
    ...
    public function getClientSubscribeLog(Request $request)
    {
        $Id = $request->business_id;

        $subscribedata = DB::table('subscriptions')
            ->join('users', 'users.business_id', '=', 'subscriptions.business_id')
            ->select('subscriptions.*', 'users.first_name', 'users.last_name', 'users.nid', 'users.contact_number', 'users.email')
            ->where('subscriptions.business_id', $Id)
            ->get();
    ...
    public function subscriptionCreate(Request $request)
    {
        ...
        if (!empty($package)) {
            $business_id = User::where('id', $user_id)->first()->business_id;
            $payment_transaction_id = $request->transaction_id;

            $this->_add_subscription($business_id, $package_id, 'offline', $payment_transaction_id, $user_id);
    ...
    public function clientPackageApproval(Request $request)
    {
        ...
        $updatedata = DB::table('subscriptions')
            ->where('business_id', '=', $bId)
            ->where('created_id', '=', $uId)
    ...
    public function pay($package_id, $form_register = null)
    {
        ...
        try {
            ...
            $count_subcriptions = Subscription::where('business_id', $business_id)
                ...
                ->where('payment_transaction_id', 'FREE')
                ->count();

            if ($count_subcriptions == 1) {

                $subcriptions_info = Subscription::where('business_id', $business_id)
                    ->where('package_id', $package_id)
                    ->where('payment_transaction_id', 'FREE')
                    ->first();
                ...
                if ($current_date < $subcriptions_info->trial_end_date) {
                    $output = ['success' => 0, 'msg' => __('Your Free Trial Period is ending soon - Upgrade now to keep enjoying POS.')];
                    return redirect()
                        ->back()
                        ->with('status', $output);
                }
            ...
            if ($count_subcriptions > 0) {
                $output = ['success' => 0, 'msg' => __('Your Free Trial Period Has Ended - Upgrade Now to Enjoy Uninterrupted Access.')];
                return redirect()
                    ->back()
                    ->with('status', $output);
            }
