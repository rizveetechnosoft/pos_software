...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        $courier_settings = empty($business->courier_settings) ? $this->businessUtil->defaultCourierSettings() : $business->courier_settings;
        ...
        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'courier_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'sms_settings', 'courier_settings', 'rp_name', 'amount_for_unit_rp',
    ...
    /**
     * Handles the testing of sms configuration Bulk Sms BD
     *
     * @return \Illuminate\Http\Response
     */
    public function testSmsConfigurationBulkSmsBD(Request $request){

        $request['sms_service'] = "bulksmsbd";
        $request['test_sms'] = true;
        ...
        try {
            ...
            $data = [
                'sms_settings' => $sms_settings,
                'mobile_number' => $sms_settings['test_bulksmsbd_number'],
                'sms_body' =>  $sms_settings['test_bulksmsbd_message'] ?  $sms_settings['test_bulksmsbd_message'] : 'This is a test SMS',
            ];
            if (! empty($sms_settings['test_bulksmsbd_number'])) {
                $response = $this->businessUtil->sendSms($data);
            } else {
                $response = "false";
            }
