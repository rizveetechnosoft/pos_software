...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $query = $query->where('p.business_id', $business_id)
        ...
        $query->select(
            DB::raw("(SELECT SUM(TSL.quantity - TSL.quantity_returned) FROM transactions 
              ...
              AND transactions.transaction_date >= '$currentMonthStart' 
              AND transactions.transaction_date <= '$currentMonthEnd') as total_sold"),
            'p.name as product'
        )->groupBy('variations.id', 'vld.location_id')->orderBy('total_sold', 'DESC')->take(5);
        $products = $query->get();
        ...
        foreach ($products as $product) {
            ...
            $product_values[] = $product->total_sold;
        }

        // Top 10 Customer 

        $topContacts = Transaction::select('contact_id', DB::raw('SUM(final_total) as total_sum'))
        ...
        return view('home.index', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin', 'product_labels', 'product_values', 'topContacts'));
    }
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            $output['total_expenses_paid']        = $this->transactionUtil->getTotalExpensesPaid($business_id      ,$start,$end,$location_id);
            ...
            $profitLossDetails  = $this->transactionUtil->getProfitLossDetails($business_id, $location_id, $start, $end);
            $trPaymentDetails   = $this->transactionUtil->getTransactionPaymentDetails($business_id, $start, $end, $location_id);
            $stockBySell        = $this->transactionUtil->getOpeningClosingStock($business_id, $end, $location_id, false, true, $filters = []);
            $stockByPurchase    = $this->transactionUtil->getOpeningClosingStock($business_id, $end, $location_id, false, false, $filters = []);

            $cashInHand         = $register_details->cash_in_hand + $register_details->total_cash - $register_details->total_cash_refund - $register_details->total_cash_expense;
            $expensesDue        = $output['total_expense'] - $output['total_expenses_paid'];
            $sellReturnDue      = $output['total_sell_return'] - $output['total_sell_return_paid'];
            $purchaseReturnDue  = $output['total_purchase_return'] - $output['total_purchase_return_paid'];
            
            $output['cash_in_hand']         = $cashInHand;
            $output['stock_by_sell']        = $stockBySell;
            $output['stock_by_purchase']    = $stockByPurchase;
            $output['tr_payment_details']   = $trPaymentDetails;
            $output['register_details']     = $register_details;
            $output['profit_loss_details']  = $profitLossDetails;
            $output['account_receivable']   = $output['invoice_due'] + $purchaseReturnDue;
            $output['account_payable']      = $purchase_details['purchase_due'] + $expensesDue + $sellReturnDue;
