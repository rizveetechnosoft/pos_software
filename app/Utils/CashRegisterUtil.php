...
class CashRegisterUtil extends Util
{
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        $register_details = $query->select(
            ...
            DB::raw("SUM(IF(pay_method='cash', IF(transaction_type='sell' OR transaction_type='purchase_return', amount, 0), 0)) as total_cash"),
            ...
            DB::raw("SUM(IF(transaction_type='sell_return', amount, 0)) as total_sell_return"),
            DB::raw("SUM(IF(transaction_type='sell_return', IF(pay_method='cash', amount, 0), 0)) as total_cash_sell_return"),
            DB::raw("SUM(IF(transaction_type='purchase_return', amount, 0)) as total_purchase_return"),
            DB::raw("SUM(IF(transaction_type='purchase_return', IF(pay_method='cash', amount, 0), 0)) as total_cash_purchase_return"),
            ...
            DB::raw("SUM(IF(t.created_at < cash_registers.created_at AND transaction_type='expense', amount, 0)) as previous_expense_paid"),
            DB::raw("SUM(IF(t.created_at < cash_registers.created_at AND transaction_type='sell', amount, 0)) as previous_due_received"),
            DB::raw("SUM(IF(t.type='expense', t.final_total, 0)) as gross_total_expense")
        )->first();
