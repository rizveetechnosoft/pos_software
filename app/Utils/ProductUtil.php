...
class ProductUtil extends Util
{
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            // Check if 'imei_numbers' key exists and has a value
            if (isset($data['imei_numbers']) && !empty($data['imei_numbers'])) {
                $this->addProductImei($data['product_id'], $data['variation_id'], $data['imei_numbers'], $transaction->id, $transaction->location_id);
            }
        }
    ...
    public function addProductImei($productId, $variationId, $imeiNumbers, $purchaseTransactionID, $locationID)
    {
        // Check if the VariationLocationDetails record exists
        $vld = VariationLocationDetails::where('variation_id', $variationId)
            ->where('product_id', $productId)
            ->where('location_id', $locationID)
            ->first();

        // Only proceed with the foreach loop if $vld exists
        if ($vld) {
            foreach ($imeiNumbers as $imei) {
                // Check if at least one IMEI number is not null
                if (!is_null($imei['imei_1'])) {
                    // Check for existence of imei_1
                    $existingImei1 = ProductImei::where('imei_1', $imei['imei_1'])
                        ->where('variation_location_detail_id', $vld->id)
                        ->whereNotNull('imei_1')
                        ->exists();

                    if ($existingImei1) {
                        // Uncomment this to store null value for duplicate imei/serial
                        // $imei['imei_1'] = null;

                        throw new Exception("IMEI 1: {$imei['imei_1']} already exists for the business.");
                    }

                    // Check for existence of imei_2 (when it's not null)
                    if (!is_null($imei['imei_2'])) {
                        $existingImei2 = ProductImei::where('imei_2', $imei['imei_2'])
                            ->where('variation_location_detail_id', $vld->id)
                            ->whereNotNull('imei_2')
                            ->exists();
                        ...
                        if ($existingImei2) {
                            ...
                            throw new Exception("IMEI 2: {$imei['imei_2']} already exists for the business.");
                        }
                    }

                    // Create the ProductImei record only if the above checks pass
                    ProductImei::create([
                        'purchase_transaction_id'      => $purchaseTransactionID,
                        'product_id'                   => $productId,
                        'variation_id'                 => $variationId,
                        'variation_location_detail_id' => $vld->id,
                        'imei_1'                       => $imei['imei_1'],
                        'imei_2'                       => $imei['imei_2'],
                    ]);
                }
    ...
    // This will delete all the related product_imei when a purchase will be delete.
    public function deleteProductImeiByPurchaseTrID($purchaseTransactionID)
    {
        return ProductImei::where('purchase_transaction_id', $purchaseTransactionID)->delete();
    }
    
    // This will reset is_sold, and the sale_transaction_id column when a sale will be delete.
    public function markProductImeiUnsoldBySaleTrID($saleTransactionID)
    {
        ProductImei::where('transaction_id', $saleTransactionID)
        ->update([
            'is_sold' => 0,
            'transaction_id' => null
        ]);
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        // Join the product_imeis table to enable IMEI searching
        $query->leftjoin('product_imeis as pi', 'VLD.id', '=', 'pi.variation_location_detail_id');
        ...
        if (! empty($search_term)) {
            ...
            if ($search_type == 'like') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    //     $query->orWhere('pi.imei_1', 'like', '%'.$search_term.'%')
                    //         ->orWhere('pi.imei_2', 'like', '%'.$search_term.'%');
                    // }

                    // Always search in imei_1 and imei_2 columns
                    // $query->orWhere('pi.imei_1', 'like', '%'.$search_term.'%')
                    //     ->orWhere('pi.imei_2', 'like', '%'.$search_term.'%');

                    // Search only un_sold imei
                    $query->orWhere('pi.is_sold', 0)
            ...
            if ($search_type == 'exact') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    // if (in_array('imei', $search_fields)) {
                    //     $query->orWhere('pi.imei_1', $search_term)
                    //         ->orWhere('pi.imei_2', $search_term);
                    // }
