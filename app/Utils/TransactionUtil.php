...
class TransactionUtil extends Util
{
    ...
    public function getTotalExpensesPaid($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = TransactionPayment::join('transactions as t', 't.id', 'transaction_payments.transaction_id')
        ...
        $total_expenses_paid = $query->first()->total_paid ?? 0;

        return $total_expenses_paid;
    }
    
    public function getTransactionPaymentDetails($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = TransactionPayment::join('transactions as t', 't.id', 'transaction_payments.transaction_id')
            ->where('t.business_id', $business_id)
            ->select(
                DB::raw('SUM(amount) as total_paid'),
                DB::raw("SUM(IF(method='cash', amount, 0)) as total_paid_cash"),

                DB::raw("SUM(IF(is_external_payment=1, amount, 0)) as external_paid"),
                DB::raw("SUM(IF(is_external_payment=1 AND method='cash', amount, 0)) as external_paid_cash"),                
                
                DB::raw("SUM(IF(is_external_payment IS NULL, amount, 0)) as register_paid"),
                DB::raw("SUM(IF(is_external_payment IS NULL AND method='cash', amount, 0)) as register_paid_cash"),
            );
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->leftJoin('product_imeis', 'transactions.id', '=', 'product_imeis.transaction_id')
                ->where('transactions.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw('GROUP_CONCAT(DISTINCT product_imeis.imei_1 ORDER BY product_imeis.id) as imei_1'), // Concatenate imei_1 values
                    DB::raw('GROUP_CONCAT(DISTINCT product_imeis.imei_2 ORDER BY product_imeis.id) as imei_2')  // Concatenate imei_2 values
                );
    ...
    /**
     * Edit transaction payment external_payment
     *
     * @param int $transactionID
     * @param bool $external_payment
     * @return object
     */
    public function updateTransactionPaymentExternalPayment(int $transactionID, bool $external_payment): object
    {
        $transactionPayment = TransactionPayment::where('transaction_id', $transactionID)->latest()->firstOrFail();
        // $transactionPayment->update(['is_external_payment' => $external_payment]);

        // Manually set and save to see if error is thrown
        $transactionPayment->is_external_payment = $external_payment;
    ...
    public function getSingleTransactionWithPayment(int $transactionID): object
{
    ...
    $paid_amount = $this->getTotalPaid($transactionID);
    $due_amount = $transaction->final_total - $paid_amount;

    // Attach the following amount to the transaction object
