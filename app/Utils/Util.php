...
class Util
{
    ...
    private function sendSmsViaBulkSmsBD($data){

        $business_id = request()->session()->get('user.business_id');
        $business_sms_setting = Business::select(['sms_settings'])
                                        ->where('id', $business_id)->first();

        $api_key    =   $business_sms_setting['sms_settings']['bulksmsbd_api_key'];
        $sender_id  =   $business_sms_setting['sms_settings']['bulksmsbd_sender_id'];
        ...
        if (isset($sms_settings['test_sms']) && $sms_settings['test_sms'] == true) {
            $request_data = [
                'api_key'   => $api_key,
                'number'    => $sms_settings['test_bulksmsbd_number'],
                'senderid'  => $sender_id,
                'message'   => $sms_settings['test_bulksmsbd_message'],
                'type'      => "text"
            ];
            $options = [];

            $response = $client->get($url.'?'.http_build_query($request_data), $options);
            ...
            if ($response->getStatusCode() == 200) {
                ...
                return "Error: HTTP ".$response->getStatusCode()." ".$response->getReasonPhrase();
            }
            ...
            $options = [];
            $client->get($url.'?'.http_build_query($request_data), $options);
        }
    }
    ...
    public function sendSms($data)
    {
        ...
        if($sms_service == 'bulksmsbd') {
            return $this->sendSmsViaBulkSmsBD($data);
        }
