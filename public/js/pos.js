$(document).ready(function() {
    ...
    // Validation function
    function validateIMEIsAndQuantities() {
        let isValid = true;

        // Iterate through each product row
        $('.product_row').each(function() {
            let rowCount = $(this).data('row_index');
            let quantity = parseInt($(`input[name='products[${rowCount}][quantity]']`).val());
            let imeis = $(`select[name='products[${rowCount}][imeis][]']`).val() || [];

            // Check if the IMEIs input exists in this row
            let imeisInput = $(`select[name='products[${rowCount}][imeis][]']`);
            if (imeisInput.length > 0) {
                let imeis = imeisInput.val() || [];

                if (imeis.length !== quantity) {
                    swal("Validation error at row " + rowCount, "The number of selected IMEIs must equal the quantity", "error");
    ...
    $('button.pos-express-finalize').click(function() {
        ...
        if (pay_method == 'credit_sale') {
            ...
            // pos_form_obj.submit();
            submitFormIfValid(); // Validate before submission
            return true;
        } else {
