$(document).ready(function() {
    ...
    $(document).on('change', '.purchase_quantity', function() {
        ...
        // Add or remove input_text for IMEI according to the number of product
        var has_imei = row.find('.has_imei').val();

        if(has_imei && has_imei==1){
            updateImeiFields(row, quantity);
        }
    });

    function updateImeiFields(row, quantity) {
        // Get the row number from the hidden input
        var row_count = row.find('.row_number').val();
        
        var imeiTd = row.find('.imei_td');
        imeiTd.empty(); // Clear the existing IMEI fields
        
        
    
        // for (var i = 0; i < quantity; i++) {
        //     var imeiIndex = i + 1;
        //     // var imeiField1 = '{!! Form::text("purchases[' + 2 + '][" + i + "][imei_1]", null, ["class" => "form-control input-sm", "placeholder" => "IMEI " + imeiIndex + "1"]); !!}';
        //     // var imeiField2 = '{!! Form::text("purchases[' + 2 + '][" + i + "][imei_2]", null, ["class" => "form-control input-sm", "placeholder" => "IMEI " + imeiIndex + "2"]); !!}';
        //     var imeiField1 = '<input type="text" >';
        //     var imeiField2 = '<input type="text" >';
    
        //     imeiTd.append(imeiField1 + '<br>' + imeiField2 + '<br>');
        // }

        if (row_count !== undefined) {
            for (var i = 0; i < quantity; i++) {
                var imeiField1 = '<input type="text" name="purchases[' + row_count + '][imei_numbers][' + i + '][imei_1]" class="form-control input-sm" placeholder="IMEI 1" required>';
                var imeiField2 = '<input type="text" name="purchases[' + row_count + '][imei_numbers][' + i + '][imei_2]" class="form-control input-sm" placeholder="IMEI 2"><br>';
    
                imeiTd.append(imeiField1 + imeiField2);
            }
        }
    }
