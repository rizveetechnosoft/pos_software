...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	{!! Form::label('products[' . $row_count .'][imeis]', __('purchase.imei_number') . ':*') !!}
	{!! Form::select('products[' . $row_count .'][imeis][]', $product->imeis, null, ['class' => 'form-control select2', 'multiple', 'required']); !!}
	@endif
	</td>
